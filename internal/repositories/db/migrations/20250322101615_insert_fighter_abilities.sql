...
-- Insert Fighter abilities that don't already exist in the database
...
-- Link Fighter abilities at appropriate levels (1st level abilities)
...
AND NOT EXISTS (
    ...
    WHERE class_name = 'Fighter' AND ability_id = abilities.id AND min_level = 1
);

-- Link Grand Mastery at level 4
INSERT INTO class_ability_mapping (class_name, ability_id, min_level)
SELECT 'Fighter', id, 4 FROM abilities 
WHERE name = 'Grand Mastery'
AND NOT EXISTS (
    SELECT 1 FROM class_ability_mapping
    WHERE class_name = 'Fighter' AND ability_id = abilities.id AND min_level = 4
);
...
AND NOT EXISTS (
    SELECT 1 FROM class_ability_mapping
    WHERE class_name = 'Fighter' AND ability_id = abilities.id AND min_level = 6
);
...
AND NOT EXISTS (
    SELECT 1 FROM class_ability_mapping
    WHERE class_name = 'Fighter' AND ability_id = abilities.id AND min_level = 7
);
