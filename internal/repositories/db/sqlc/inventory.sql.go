...
const recalculateInventoryWeight = `-- name: RecalculateInventoryWeight :exec
UPDATE inventories
SET current_weight = (
...
func (q *Queries) RecalculateInventoryWeight(ctx context.Context, id int64) error {
	_, err := q.exec(ctx, q.recalculateInventoryWeightStmt, recalculateInventoryWeight, id)
	return err
}
...
func (q *Queries) UpdateInventoryWeight(ctx context.Context, arg UpdateInventoryWeightParams) error {
	_, err := q.exec(ctx, q.updateInventoryWeightStmt, updateInventoryWeight, arg.CurrentWeight, arg.ID)
	return err
}
